...
Page({
  data: { rows: [], month: '', result: null, sheet: false, sheetHours: '', sheetLessons: '', sheetUnit: 'hours', sheetError: '', sheetDuplicate: false, sheetHeight: 480, keyboardHeight: 0, gradeIndex: 0, classIndex: 0, grades: GRADES, classes: [CLASS_NAMES[1], CLASS_NAMES[2], CLASS_NAMES[3]], focusKey: '', keyboardOpen: false },
  onLoad() {
    this.unsubscribe = session().subscribe(() => this.render())
    this.keyboard = ({ height }) => this.setData({ keyboardOpen: height > 0, keyboardHeight: height, sheetHeight: Math.max(200, Math.min(480, (wx.getWindowInfo?.().windowHeight || 650) - height - 24)) })
  ...
  render() {
    const s = session()
    this.setData({ ...sessionView(s), ...(s.identifying ? { sheet: false, focusKey: '', sheetHours: '', sheetLessons: '', sheetError: '' } : {}) })
    leaveGuard(s.hasWork && s.resultState !== 'saved')
  },
  changeMonth(e) { run(() => session().setMonth(e.detail.value)) },
  openSheet() { if (!session().busy) { this.setData({ sheet: true, sheetHours: '', sheetLessons: '', sheetError: '', sheetUnit: 'hours' }); this.checkDuplicate() } },
  ...
  add() {
    run(() => {
      ...
      this.setData({ sheet: false, focusKey: '' })
      wx.hideKeyboard()
      wx.nextTick(() => { this.setData({ focusKey: key }); wx.pageScrollTo({ selector: `#entry-${key}`, duration: 250 }) })
    })
  },
  ...
  calculate() { run(() => {
    ...
    catch (e) { const row = session().rows.find(r => r.error); if (row) { this.setData({ focusKey: row.key }); wx.pageScrollTo({ selector: `#entry-${row.key}`, duration: 250 }) }; throw e }
    wx.hideKeyboard(); wx.nextTick(() => wx.pageScrollTo({ selector: '#result', duration: 250 }))
  }) },
