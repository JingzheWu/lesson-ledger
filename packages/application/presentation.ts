...
export function resultView(result: Result, rules: Rules = rulesSnapshot(), record?: SavedRecord) {
  ...
  return {
    ...
    tiers: result.tiers.map(t => ({ id: t.tierId, name: rules.tiers[t.tierId].name, hours: hours(t.hoursHundredths), amount: money(t.feeCents),
      composition: t.details.map(d => `${label(d.gradeId, d.classSize)} ${hours(d.hoursHundredths)}小时`).join(' ＋ ') || '本阶梯未使用',
