...
function parseDecimal(text: string, decimals: number, blankIsZero: boolean): Parsed {
  ...
  const integer = BigInt(whole || '0') * 10n ** BigInt(decimals) + BigInt(fraction.padEnd(decimals, '0'))
  if (integer > BigInt(Number.MAX_SAFE_INTEGER)) return { error: '数值过大，请检查输入' }
...
export function lessons(value: number): string {
  const raw = (BigInt(value) * 5n).toString().padStart(4, '0')
...
export function calculate(input: Input, config: Config, calculatedAt: string): Result {
  ...
  for (const entry of entries) {
    ...
    while (remaining > 0) {
      ...
      const numerator = BigInt(allocated) * BigInt(rateCents) * BigInt(multiplier)
      const feeCents = safe(Number((numerator + 500n) / 1000n))
